const unban = function(client, Data, fileName, DBB) {
    const times = Data.getData("time", fileName, "block");

    if(times && DBB.Core.typeof(times) == "object") {
        for (const userID in times) {
            ...
            if(Date.now() >= (parseInt(info[0]) || 0)) {
                ...
                if(server) server.bans.remove(parseInt(userID) ? userID + "" : "", info[2] ? info[2] + "" : "");
...
module.exports = {
    ...
    inputs: [
        ...
        },
        {
            ...
            "name": "Delete User Message History (Seconds)",
            "description": "Acceptable Types: Number, Unspecified\n\nDescription: The number of seconds of banned user's messages to delete. (OPTIONAL)",
    ...
    init(DBB, fileName) {
        unban(DBB.DiscordJS.client, DBB.Blocks.Data, fileName, DBB);
    },

    code(cache, DBB) {
        const server = this.GetInputValue("server", cache);
        const user = this.GetInputValue("target", cache);
        const deleteMessageSeconds = parseInt(this.GetInputValue("delete_user_message_history", cache));
