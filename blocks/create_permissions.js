module.exports = {
    ...
    description: "Creates a set of permissions to apply to a role or channel.",
    ...
    outputs: [
        ...
        },
        {
            "id": "permissions",
            "name": "Permissions",
            "description": "Type: Object\n\nDescription: The set of permissions created.",
            "types": ["object"]
        }
    ],
    ...
    code: function(cache) {
        ...
        for (const [rawFlag, flag] of flags) {
            const perm = this.GetOptionValue(rawFlag, cache);

            if (perm && ["allow", "inherit", "deny"].includes(perm))
                perms[perm].add(flag);
        }

        this.StoreOutputValue(perms, "permissions", cache);
