module.exports = {
    ...
    async code(cache) {
        ...
        if(em !== undefined) {
            inter.editReply({ content: msg, embeds: [em], files: file ? [file] : null, components: components, ephemeral: awnser });
            this.StoreOutputValue(message, "message", cache);
            this.RunNextBlock("action", cache);
        } else {
            inter.editReply({ content: msg, embeds: [em], components: components, ephemeral: awnser });
            this.StoreOutputValue(message, "message", cache);
            this.RunNextBlock("action", cache);
        }
    }
